...
public class GameManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        playerVsComputer.gameObject.SetActive(false);
    ...
    void ResolveToss()
    {
        ...
        {
            bool computerBowl = Random.Range(0, 2) == 1;
    ...
    void PlayerBat(int playerNumber)
    {
        ...
        UpdateChoiceDisplay(playerNumber, computerNumber);
    ...
    void PlayerBowl(int playerBowl)
    {
        ...
        UpdateChoiceDisplay(playerBowl, computerBat);
        Batting.SetActive(false);
        Bowling.SetActive(true);
    ...
    void StartInnings()
    {
        ...
        Batting.SetActive(playerBatting);
        Bowling.SetActive(!playerBatting);
        UpdateLiveScore();
        UpdateTargetText();
    }
    ...
    void EndMatch()
    {
        ...
        else
        {
            if (playerScore > computerScore)
            {
                if (playerBatting)
                    msg = "CONGRATULATIONS!\nYOU WON THE MATCH\nBY "
                          + (maxWickets - wickets) + " WICKETS";
                else
                    msg = "CONGRATULATIONS!\nYOU WON THE MATCH\nBY "
                          + (playerScore - computerScore) + " RUNS";
            ...
            else if (computerScore > playerScore)
            {
                if (playerBatting)
                    msg = "YOU LOST THE MATCH\nBY "
                          + (computerScore - playerScore) + " RUNS\nTRY AGAIN!";
                else
                    msg = "YOU LOST THE MATCH\nBY "
                          + (maxWickets - wickets) + " WICKETS\nTRY AGAIN!";
    ...
    // ================= EXIT PANEL =================
    public void OnPlayPressed() => ExitPanel.SetActive(false);
    ...
    // ================= CHOICE DISPLAY =================
    Sprite GetShotIcon(int number)
    {
        return (number <= 3) ? lowIcon : highIcon;
    }

    void UpdateChoiceDisplay(int playerNum, int computerNum)
    {
        playerChoiceImage.gameObject.SetActive(true);
        computerChoiceImage.gameObject.SetActive(true);
        vsText.gameObject.SetActive(true);
        playerVsComputer.gameObject.SetActive(true);

        playerChoiceImage.sprite = GetShotIcon(playerNum);
        computerChoiceImage.sprite = GetShotIcon(computerNum);

        playerChoiceText.text = playerNum.ToString();
        computerChoiceText.text = computerNum.ToString();
    }
